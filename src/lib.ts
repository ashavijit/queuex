...
export class QueueX {
      ...
      constructor(config: QueueXConfig) {
            try {
                  ...
                  throw new Error(
                        `Failed to initialize QueueX: ${err instanceof Error ? err.message : String(err)}`
                  );
            }
      }
      ...
      startWorker(
            ...
            worker.start(queueName, processor).catch((err) =>
      ...
      async shutdown(): Promise<void> {
            try {
                  ...
                  throw new Error(
                        `Failed to shutdown QueueX: ${err instanceof Error ? err.message : String(err)}`
                  );
            }
      }

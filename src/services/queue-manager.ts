...
import { Queue, QueueOptions, QueueStrategy } from '../models/queue';
...
export class QueueManager {
      ...
      constructor(
            ...
            this.processOverdueJobs().catch((err) =>
                  console.error('Overdue job processing failed:', err)
            );
      }
      ...
      async enqueue(queueName: string, data: any, options: JobOptions = {}): Promise<Job> {
            ...
            try {
                  await this.storage.enqueueJob(job, queue.options.strategy || QueueStrategy.FIFO);
      ...
      async getJob(queueName: string): Promise<Job | null> {
            try {
                  ...
                  const job = await this.storage.getNextJob(
                        queueName,
                        queue.options.strategy || QueueStrategy.FIFO
                  );
                  if (!job) return null;
                  ...
                  throw new Error(
                        `Failed to get job from queue ${queueName}: ${err instanceof Error ? err.message : String(err)}`
                  );
            }
      }
      ...
      async unscheduleJob(jobId: string): Promise<void> {
            ...
            try {
                  ...
                  throw new Error(
                        `Failed to unschedule job ${jobId}: ${err instanceof Error ? err.message : String(err)}`
                  );
            }
      }
      ...
      async updateJobDelay(jobId: string, newDelay: number): Promise<Job> {
            ...
            try {
                  ...
                  throw new Error(
                        `Failed to update delay for job ${jobId}: ${err instanceof Error ? err.message : String(err)}`
                  );
            }
      }
      ...
      private getScheduledAt(options: JobOptions): number | undefined {
            ...
            if (options.cron) {
                  try {
                        ...
                        throw new Error(
                              `Invalid cron expression: ${options.cron} - ${err instanceof Error ? err.message : String(err)}`
                        );
                  }
            }
            return undefined;
